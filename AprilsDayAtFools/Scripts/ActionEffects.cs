...
namespace AprilsDayAtFools
{
    ...
    public class CasterPriorityRootActionEffect : EffectSO
    {
        public EffectInfo[] effects;
        public override bool PerformEffect(CombatStats stats, IUnit caster, TargetSlotInfo[] targets, bool areTargetSlots, int entryVariable, out int exitAmount)
        {
            EffectInfo[] effectInfoArray = effects;
            exitAmount = 0;
            CombatManager.Instance.AddPriorityRootAction(new EffectAction(effectInfoArray, caster, 0));
            return true;
        }
        public static CasterPriorityRootActionEffect Create(EffectInfo[] e)
        {
            CasterPriorityRootActionEffect instance = CreateInstance<CasterPriorityRootActionEffect>();
            instance.effects = e;
            return instance;
        }
    }
    ...
    public class UIActionAction : CombatAction
    {
        ...
        public UIActionAction(CombatAction a) => ex = a;

        public override IEnumerator Execute(CombatStats stats)
        {
            CombatManager.Instance.AddUIAction(ex);
            yield return null;
        }
    }
